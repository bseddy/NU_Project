...
# The site https://regex101.com was a great learning tool - you can validate input to test the regex

if re.match('(?=.*[A-Z]{2})(?=.*[a-z]{2})(?=.*[0-9]{2})(?=.*[!@#$%^&*()?<>_+=]{2}).{8,}', user_password):
    print('Password meets criteria for a STRONG password')

# else statement, ALL defined password criteria must match, otherwise tell user their password is not strong enough
else:
    print('Password DOES NOT meet criteria for a strong password!')
